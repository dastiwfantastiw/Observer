...
fRtlAllocateHeap RtlAllocateHeap = (fRtlAllocateHeap)GetProcAddress(LoadLibraryA("ntdll"), "RtlAllocateHeap");
fRtlFreeHeap RtlFreeHeap = (fRtlFreeHeap)GetProcAddress(LoadLibraryA("ntdll"), "RtlFreeHeap");

} // namespace memory

bool memory::IsBadReadAddress(void* address, uint32_t* szPage)
{
    if (((uint32_t)address & 0xff000000) ==  0x7f000000)
        return true;
